...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        imgView = [[UIImageView alloc]initWithFrame:CGRectMake(frame.size.width/2-45, frame.size.width/2-90, 90, 90)];
        [imgView setBackgroundColor:[UIColor whiteColor]];
        [imgView.layer setCornerRadius:45];
        [imgView.layer setBorderColor:[UIColor grayColor].CGColor];
        [imgView.layer setBorderWidth:2];
        [self addSubview:imgView];
        
        forgetButton = [[UIButton alloc]initWithFrame:CGRectMake(frame.size.width/2+30, frame.size.height/2+220, 120, 30)];
