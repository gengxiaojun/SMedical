    ...
    if (touch) {
        ...
        for (int i=0; i<buttonArray.count; i++) {
            ...
            if (CGRectContainsPoint(buttonTemp.frame,touchPoint)) {
                ...
                NSDictionary * dict = [NSDictionary dictionaryWithObjectsAndKeys:[[NSString  alloc]initWithFormat:@"%f",point.x],@"x",[[NSString alloc] initWithFormat:@"%f",point.y],@"y", nil];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (touch) {
        ...
        for (int i=0; i<buttonArray.count; i++) {
            GesturePasswordButton * buttonTemp = ((GesturePasswordButton *)[buttonArray objectAtIndex:i]);
            if (CGRectContainsPoint(buttonTemp.frame,touchPoint)) {
                if ([touchedArray containsObject:[[NSString alloc] initWithFormat:@"num%d",i]]) {
                    lineEndPoint = touchPoint;
                    [self setNeedsDisplay];
                    return;
                }
                [touchedArray addObject:[[NSString alloc] initWithFormat:@"num%d",i]];
                [buttonTemp setSelected:YES];
                [buttonTemp setNeedsDisplay];
                CGRect frameTemp = buttonTemp.frame;
                CGPoint point = CGPointMake(frameTemp.origin.x+frameTemp.size.width/2,frameTemp.origin.y+frameTemp.size.height/2);
                NSDictionary * dict = [NSDictionary dictionaryWithObjectsAndKeys:[[NSString alloc] initWithFormat:@"%f",point.x],@"x",[[NSString alloc] initWithFormat:@"%f",point.y],@"y",[[NSString alloc] initWithFormat:@"%d",i],@"num", nil];
                [touchesArray addObject:dict];
                break;
            }
        }
